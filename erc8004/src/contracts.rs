...
sol! {
    ...
    contract IdentityRegistry {
        ...
        /// Register a new agent (no URI, set later via setAgentURI).
        function register() external returns (uint256 agentId);

        /// Register a new agent with a URI.
        function register(string agentURI) external returns (uint256 agentId);

        /// Register a new agent with a URI and initial metadata.
        ...
        function getAgentWallet(uint256 agentId) external view returns (bytes);
